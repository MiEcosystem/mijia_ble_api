...
mible_status_t mible_record_create(uint16_t record_id, uint8_t len)
{
	...
	if(PDM_u8GetSegmentCapacity() > 0)
	{
		MI_LOG_INFO("mible_record_create() cap: %d:\r\n", PDM_u8GetSegmentCapacity());
		err = MI_SUCCESS;
	}
	else
	{
		err = MI_ERR_NO_MEM;
		MI_LOG_INFO("mible_record_create() err\r\n");
	}
...
mible_status_t mible_record_delete(uint16_t record_id)
{
    ...
    PDM_vDeleteDataRecord(record_id);

    mible_arch_evt_param_t par;
    par.record.id = record_id;
    par.record.status = (mible_status_t)err;

    mible_arch_event_callback(MIBLE_ARCH_EVT_RECORD_DELETE, &par);
...
mible_status_t mible_record_read(uint16_t record_id, uint8_t* p_data,
    uint8_t len)
{
    uint8_t err = MIBLE_ERR_UNKNOWN;
    uint16_t length;
    
    if(PDM_bDoesDataExist(record_id, &length) == true)
    {
    	if(length == len)
    	{
			/* This is assuming that *lengthPtr is initialized with maximum size of buffer */
			if (PDM_eReadDataFromRecord(record_id, p_data, len, &length) == PDM_E_STATUS_OK)
			{
				err =  MI_SUCCESS;
			}
    	}
    ...
    uint8_t len)
{
    ...
    if (PDM_eSaveRecordData(record_id, p_data, (uint16_t)len, false) ==  PDM_E_STATUS_OK)
    {
    	err =  MI_SUCCESS;
    }
