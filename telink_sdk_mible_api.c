...
u8 telink_ble_mi_app_event(u8 sub_code , u8 *p, int n)
{
	...
	}else if (sub_code == HCI_CMD_DISCONNECTION_COMPLETE){
		gap_evt_t = MIBLE_GAP_EVT_DISCONNET;
...
u32 A_debug_mi_event0=0;
u32 A_debug_mi_event1=0;
...
u8 telink_ble_mi_event_callback(u8 opcode,u8 *p)
{
	...
	if(opcode == ATT_OP_WRITE_REQ || opcode == ATT_OP_WRITE_CMD ){
		...
		if((gAttributes[handle_val].perm) & ATT_PERMISSIONS_READ){
...
extern _attribute_data_retention_ u8	tbl_mac [6];
...
// user function ,and will call by the mi part 
_attribute_data_retention_   rf_packet_adv_t mi_adv_packet;
...
mible_status_t telink_ble_mi_gatts_service_init(mible_gatts_db_t *p_server_db)
{
	...
	for(int i=0;i<(p_srv->char_num);i++){
		...
		if(p_char_db[i].wr_author ){
			(my_Attributes[handle].perm) &= ~(ATT_PERMISSIONS_AUTHOR_WRITE);
		}else{
			(my_Attributes[handle].perm) |= ATT_PERMISSIONS_AUTHOR_WRITE;
		}
		if(p_char_db->rd_author ){
			(my_Attributes[handle].perm) &=~(ATT_PERMISSIONS_AUTHOR_READ);
		}else{
			(my_Attributes[handle].perm) |= ATT_PERMISSIONS_AUTHOR_READ;
		}
	}
...
void advertising_init(void)
{
    ...
    mible_gap_adv_data_t adv_data={0};
    // add flags
    ...
    *(adv_data.adv_data + 2) = 0x06;
    ...
    mible_gap_adv_data_set(adv_data.adv_data,adv_data.adv_len,adv_data.scan_rsp_data,adv_data.scan_rsp_len);
...
void advertising_start(void){
    mible_gap_adv_param_t adv_param =(mible_gap_adv_param_t){
        ...
        .adv_interval_min = 30*1000/625,
        .adv_interval_max = 40*1000/625,
    };
...
u32  mi_set_log_end_tick =0;
void mi_set_log_loop_proc()
{
	if(mi_set_log_end_tick && clock_time_exceed(mi_set_log_end_tick,10000*1000)){
		mi_set_encrypt_busy_flag(0);
		mi_set_log_end_tick =0;
	}
}
        ...
        mible_std_auth_evt_param_t* p_param)
{
	...
	if(p_param->result.state == LOGIN_SUCC ){
		mi_set_log_end_tick = clock_time()|1;
