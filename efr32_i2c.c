...
static mible_handler_t i2cCallback;
...
static const LDMA_Descriptor_t p2mLink =
LDMA_DESCRIPTOR_LINKREL_P2M_BYTE(NULL, NULL, 1UL, 1UL);
static const LDMA_Descriptor_t m2pSingle =
LDMA_DESCRIPTOR_SINGLE_M2P_BYTE(NULL, NULL, 1UL);
static const LDMA_Descriptor_t m2pLink =
LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(NULL, NULL, 1UL, 1UL);
static const LDMA_Descriptor_t wriSingle =
LDMA_DESCRIPTOR_SINGLE_WRITE(NULL, NULL);
static const LDMA_Descriptor_t wriLink =
LDMA_DESCRIPTOR_LINKREL_WRITE(NULL, NULL, 1UL);
static const LDMA_Descriptor_t syncLink =
...
void LDMA_IRQHandler(void)
{
    ...
    if (pending & LDMA_IF_ERROR) {
        ...
        pending = (LDMA->STATUS & ~_LDMA_STATUS_CHERROR_MASK)
...
void I2C_ISR(void)
{
    ...
    iic_event_t iic_status;           // Transfer status

    if (flags & I2C_IF_ARBLOST) {
        iic_status = IIC_EVT_ADDRESS_NACK;
        transferResult = i2cTransferArbLost;
        I2C_NUM->CMD = I2C_CMD_ABORT;
        if (i2cCallback != NULL)
            i2cCallback(&iic_status);
...
mible_status_t mible_iic_init(const iic_config_t * p_config, mible_handler_t handler)
{
    ...
    GPIO_PinModeSet((GPIO_Port_TypeDef) (m_p_config->scl_port), m_p_config->scl_pin,
            gpioModeWiredAnd, 1);
    GPIO_PinModeSet((GPIO_Port_TypeDef) (m_p_config->sda_port), m_p_config->sda_pin,
            gpioModeWiredAnd, 1);
...
mible_status_t mible_iic_rx(uint8_t addr, uint8_t * p_in, uint16_t len)
{
    ...
    slaveAddrR = addr << 1 | I2C_READ_BIT;     // Slave address for read
    *descTx = m2pSingle;
 ...
 * and the iic event should be passed as a argument. 
 * */
mible_status_t mible_iic_tx_rx(uint8_t addr, uint8_t *p_out, uint8_t outlen,
        uint8_t * p_in, uint16_t inlen)
{
    LDMA_TransferCfg_t xferTx;
    LDMA_TransferCfg_t xferRx;
    LDMA_Descriptor_t *descTx = &descLinkTx[0];
